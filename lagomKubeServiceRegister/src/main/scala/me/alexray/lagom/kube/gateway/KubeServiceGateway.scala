 ...
 *
 * based on https://github.com/lagom/lagom/blob/1.3.3/dev/service-registry/service-locator/src/main/scala/com/lightbend/lagom/gateway/ServiceGateway.scala
 *
 */


package me.alexray.lagom.kube.gateway
...
import me.alexray.lagom.kube.discovery.KubeServiceRegistryActor._
...
case class KubeServiceGatewayConfig(port: Int)

@Singleton
class KubeServiceGatewayFactory @Inject()(lifecycle: ApplicationLifecycle, config: KubeServiceGatewayConfig,
                                          @Named(KubeServiceRegistryModule.KUBE_SERVICE_REGISTRY_ACTOR) registry: ActorRef)
{
  def start(): KubeServiceGateway = {
    new KubeServiceGateway(lifecycle, config, registry)
  }
}

class KubeServiceGateway(lifecycle: ApplicationLifecycle, config: KubeServiceGatewayConfig, registry: ActorRef) {

  private implicit val timeout = Timeout(5.seconds)
  private val log = LoggerFactory.getLogger(classOf[KubeServiceGateway])

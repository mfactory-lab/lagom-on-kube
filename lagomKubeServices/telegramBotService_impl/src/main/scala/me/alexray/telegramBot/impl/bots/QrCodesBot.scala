...
/**
  ...
  */
class QrCodesBot(token: String) extends BotWithToken(token) with Polling with Commands with ChatActions {

  on("/qr") { implicit message => args =>

    val url = "http://api.qrserver.com/v1/create-qr-code/?data=" +
      URLEncoder.encode(args mkString " ", "UTF-8")

    logger.info(s"url: $url")
    println(s"url: $url")

    val response = Http().singleRequest(HttpRequest(uri = Uri(url)))

    response.failed.foreach { t =>
      logger.info(s"failded: ${t.getLocalizedMessage}")
    }
